...
class MessengerBotChatTaskWorld(World):
    ...
    def parley(self):
        ...
        a = self.agent.act()
        if x == 1:
           a = {'episode_done': False, 'text': 'start', 'payload': 'thecookiedistilbert'}
        elif x == 2:
            a = {'episode_done': False, 'text': 'start', 'payload': 'thecookiepyabsa'}
        if a is not None:
            print("a", a)
            if '[DONE]' in a['text']:
                self.episodeDone = True
                ...
                if x == 0:
                    tokenizer_bert = BertTokenizer.from_pretrained('./customworld/mental_bert_swmh_improved')
                    model_bert = BertForSequenceClassificationImproved.from_pretrained(
                        './customworld/mental_bert_swmh_improved')
                    model_bert.to(device)
                elif x == 1:
                    print("xxxxxxxxxx", x)
                    tokenizer_bert = AutoTokenizer.from_pretrained("bdotloh/distilbert-base-uncased-empathetic-dialogues-context")

                    model_bert = AutoModelForSequenceClassification.from_pretrained("bdotloh/distilbert-base-uncased-empathetic-dialogues-context")
                    model_bert.to(device)
                elif x == 2:
                    # tokenizer_bert = AutoTokenizer.from_pretrained("yangheng/deberta-v3-large-absa-v1.1")
                    tokenizer_bert = AutoTokenizer.from_pretrained('./customworld/pyabsa_model')

                    # model_bert = AutoModelForSequenceClassification.from_pretrained("yangheng/deberta-v3-large-absa-v1.1")
                    model_bert = AutoModelForSequenceClassification.from_pretrained('./customworld/pyabsa_model')
                    model_bert.to(device)

                class_name, prob, confidences = self.inference(model_bert, tokenizer_bert, a['text'])

                if x == 0:
                    # filename = f"confidences/{a['payload']}.csv"
                    filename = f"confidences/thecookie.csv"
                elif x == 1:
                    filename = f"confidences/thecookiedistilbert.csv"
                elif x == 2:
                    filename = f"confidences/thecookiepyabsa.csv"
    ...
    def get_largest_label(self, filename):
        ...
        # if len(df) < 5 or len(df) % 5 != 0:
        #     return label_with_highest_avg, 0
        latest_values = df.tail(5).to_dict("list")
        max_average = float("-inf")
        print("latest value", latest_values.items())

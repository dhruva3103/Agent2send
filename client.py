...
class BrowserHandler(BaseHTTPRequestHandler):
    ...
    def _interactive_running(self, reply_text, cookie):
        data = {}
    ...
    def do_POST(self):
        ...
        """
        cookie, not_set = self.set_or_get_cookie()

        if self.path == '/interact':
            content_length = int(self.headers['Content-Length'])
            body = self.rfile.read(content_length)
            self._interactive_running(body, cookie)
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            if not not_set:
                self.send_header("Set-Cookie", f"sid={cookie}")
            self.end_headers()
            ...
            self._interactive_running(b"[RESET]", cookie)
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            if not not_set:
                self.send_header("Set-Cookie", f"sid={cookie}")
    ...
    def set_or_get_cookie(self):
        cookies = self.parse_cookies(self.headers["Cookie"])
        if "sid" in cookies:
            return cookies["sid"], True
        sid = self.generate_sid()
        return sid, False

    def generate_sid(self):
        return "".join(str(randint(1,9)) for _ in range(50))
...
if __name__ == "__main__":
    ...
    ws = websocket.WebSocketApp(
        "ws://model-server:{}/websocket".format(port),
